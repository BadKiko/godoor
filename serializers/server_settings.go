...
type ServerSettingsSerializer struct {
	...
	CustomTheme  interface{} `json:"custom_theme"` // Note: original uses 'custom_theme' not 'custom_space_theme'
...
func SerializeServerSettings() ServerSettingsSerializer {
	return ServerSettingsSerializer{
		ShoppingMinAutosyncInterval:   fmt.Sprintf("%d", config.ShoppingMinAutosyncInterval),
		...
		LogoColor512: nil,
		LogoColorSvg: nil,
		CustomTheme:  nil, // Note: original uses 'custom_theme' not 'custom_space_theme'
		NavLogo:      nil,
		NavBgColor:   nil,
	}
}
